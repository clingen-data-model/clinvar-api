...
def main(argv):
    ...
    generate_subparser.add_argument("--filename", required=True, help="ClinVar submission MS Excel file")
    generate_subparser.add_argument("--prettyjson", default="true",
                        choices=["true", "false"],
                        help="Set to False")

    submit_subparser = subparsers.add_parser("submit")
    submit_subparser.add_argument("--directory", type=str, help="Submit all files from this directory")
    submit_subparser.add_argument("--file", type=str, help="Submit this file")
    submit_subparser.add_argument("--config", required=True, help="Config file for clinvar-api")

    opts = parser.parse_args(argv)

    input_filename = opts.filename
    ...
    if opts.subcommand == "generate":
        ...
        if opts.directory and opts.file:
            raise RuntimeError("Cannot use --directory and --file at the same time")
        raise RuntimeError("submit not yet implemented")
